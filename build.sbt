...
// play-json resolves. jackson-core below 2.18.8 -- and again on 2.19.0 through 2.21.3 -- applies
// maxNumberLength to the digits within each chunk fed to the non-blocking parser rather than to
// the number accumulated across feeds, so a number split across `feedInput` calls is not bounded
// at all and no chunk ever has to exceed the limit (GHSA-r7wm-3cxj-wff9).
//
